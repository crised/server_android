...
public class TelematicHandler implements ResponseHandler {
    ...
    private void deleteOld() {
        ...
        try {
            ...
            if(old.getCount() > 0) {
                ...
                db.delete(TelematicMember.Members.MEMBER_NAME, where, null);
    ...
    private int parseTelematicEntity(HttpEntity entity) throws IOException {
        ...
        //TODO use GSON to parse the String
        String[] members = membersS.split("\\},\\{");
        ContentValues memberEntry;
        int counter = 0;
        for(String member: members) {
            String id = member.substring(member.indexOf("\"id\":") + 5, member.indexOf(",\"name"));
            String name = member.substring(member.indexOf("\"name\":") + 7, member.indexOf("\",\"email"));
            String email = member.substring(member.indexOf("\"email\":") + 8, member.indexOf("\",\"phoneNumber"));
            String phoneNumber = member.substring(member.indexOf("\"phoneNumber\":") + 15, member.indexOf("\"}"));
            memberEntry = new ContentValues();
            memberEntry.put(TelematicMember.Members.MEMBER_ID, Integer.parseInt(id));
